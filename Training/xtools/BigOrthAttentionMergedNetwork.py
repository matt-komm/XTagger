...
class BigOrthAttentionMergedNetwork(xtools.NominalNetwork):
    def __init__(self,featureDict):
        ...
        #### CAND ####
        self.cand_conv = []
        for i,filters in enumerate([128,32,32,16]):
        ...
        self.cand_attention = []
        for i,filters in enumerate([128,32,32,10]):
    ...
    def extractFeatures(self,globalvars,cpf,npf,sv,muon,electron,gen=None):
        ...
        global_conv = addConvFeatures
        global_attention = addConvFeatures
        ...
        cand_conv = self.applyLayers(self.addToConvFeatures(cand,global_conv),self.cand_conv)
        cand_attention = self.applyLayers(self.addToConvFeatures(cand,global_attention),self.cand_attention)
        ...
        def orth(x):
            ...
            for i in range(x.shape[2]):
                if i==0:
                    ...
                    v = x[:,:,i]
                    for j in range(i-1):
                        #v -= tf.tensordot(x[:,:,i],x[:,:,j],axes=1)*x[:,:,j]
                        w = tf.reduce_sum(tf.multiply(x[:,:,i],result[j]),axis=1)/tf.reduce_sum(tf.square(result[j]),axis=1)
                        v -= tf.einsum('bi,b->bi',result[j],w)

                    result.append(v)
            return tf.stack(result,axis=2)
            
        cand_attention = keras.layers.Lambda(orth)(cand_attention)
...
network = BigOrthAttentionMergedNetwork
