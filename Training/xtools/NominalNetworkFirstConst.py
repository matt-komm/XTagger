...
class NominalNetworkFirstConst():
    ...
    def extractFeatures(self,globalvars,cpf,npf,sv,muon,electron,gen=None):
        ...
        sliced_cpf = keras.layers.Lambda(lambda x: x[:,0:1,:])(cpf)
        sliced_npf = keras.layers.Lambda(lambda x: x[:,0:1,:])(npf)

        #cpf_conv = self.applyLayers(self.cpf_preproc(cpf),self.cpf_conv)
        #npf_conv = self.applyLayers(self.npf_preproc(npf),self.npf_conv)

        cpf_conv = self.applyLayers(self.cpf_preproc(sliced_cpf),self.cpf_conv)
        npf_conv = self.applyLayers(self.npf_preproc(sliced_npf),self.npf_conv)
...
network = NominalNetworkFirstConst
