    ...
    public OpKernel
{
    ...
    public:
        explicit LxyWeightsOp(OpKernelConstruction* context): 
            OpKernel(context)
        {
            OP_REQUIRES_OK(
                context,
            ...
            for (auto histName: histNames)
            {
                ...
                TH1* hist = dynamic_cast<TH1*>(rootFile.Get(histName.c_str())->Clone());
                if (not hist)
                {
                    context->CtxFailureWithWarning(__FILE__, __LINE__, Status(error::INVALID_ARGUMENT,
                ...
                hist->SetDirectory(0);
                hists.push_back(std::shared_ptr<TH1>(hist));
            }
        }
        
        
        virtual ~LxyWeightsOp()
        { 
            RootMutex::Lock lock = RootMutex::lock();
            hists.clear();
        }
        
        float computeWeight(unsigned int classIndex, float value)
        {
            TH1* hist = hists[classIndex].get();
        ...
        void Compute(OpKernelContext* context)
        {
            ...
            for (unsigned int ibatch = 0; ibatch < num_batches; ++ibatch)
            {
                ...
                float varValue = value(ibatch*value_size+varIndex[0]);
...
REGISTER_KERNEL_BUILDER(Name("LxyWeights").Device(DEVICE_CPU),LxyWeightsOp)
